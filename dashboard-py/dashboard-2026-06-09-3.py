...
def fig_engineering_advantage_diverging_bar(subj_df: Optional[pd.DataFrame]) -> go.Figure:
    """
    Final Domain Strategy chart with Anglosphere split into:
    - United States
    - United Kingdom
    - Other Anglosphere
    - Asia
    ...
    Calculation:
        engineering_advantage = Arts & Humanities rank - Engineering rank

    Interpretation:
        positive = Engineering ranks better than Arts & Humanities
        negative = Arts & Humanities ranks better than Engineering
        zero = balanced
    """

    placeholder_title = "Median Engineering advantage by policy group"

    if subj_df is None or subj_df.empty:
        return _placeholder_fig(
            "Subject ranking data was not found. Add the_subjects_2026.csv or the_subjects.csv.",
            title=placeholder_title,
        )

    df = clean_columns(subj_df.copy())

    # -------------------------------------------------------------------------
    # 1. Chart-specific grouping: split the Anglosphere
    # -------------------------------------------------------------------------
    DOMAIN_GROUP_ORDER = [
        "United States",
        "United Kingdom",
    ...
    DOMAIN_GROUP_COLORS = {
        ...
        "United Kingdom": "#56B4E9",
        "Other Anglosphere": "#8DA0CB",
        "Asia": "#D55E00",
        "Europe": "#009E73",
        "Rest of World": "#CC79A7",
    }
    ...
    def assign_domain_group(country) -> str:
        c = normalize_country(country)
        ...
        if c in OTHER_ANGLOSPHERE:
            return "Other Anglosphere"
    ...
    # -------------------------------------------------------------------------
    # 2. Standardise institution and country columns
    # -------------------------------------------------------------------------
    name_col = first_existing(
        df,
        [
    ...
    df["country"] = df["country"].apply(normalize_country)
    df["domain_group"] = df["country"].apply(assign_domain_group)

    # -------------------------------------------------------------------------
    # 3. Use latest year only, if year exists
    # -------------------------------------------------------------------------
    latest_year_text = ""
    ...
    if "year" in df.columns:
        ...
        if year_num.notna().any():
            latest_year = int(year_num.max())
            df = df.loc[year_num == latest_year].copy()
            latest_year_text = f"Latest year used: {latest_year}"

    # -------------------------------------------------------------------------
    # 4. Identify subject column
    # -------------------------------------------------------------------------
    ...
    engineering_mask = df["_subject_text"].str.contains(
        r"\bengineering\b",
        regex=True,
        na=False,
    )
    ...
    arts_mask = (
        df["_subject_text"].str.contains("arts", na=False)
        & df["_subject_text"].str.contains("human", na=False)
    )

    df = df.loc[engineering_mask | arts_mask].copy()

    if df.empty:
        return _placeholder_fig(
            "Engineering and Arts & Humanities rows were not found in the subject data.",
            title=placeholder_title,
        )

    df["domain"] = np.where(
        df["_subject_text"].str.contains(r"\bengineering\b", regex=True, na=False),
        "Engineering",
        "Arts & Humanities",
    )

    # -------------------------------------------------------------------------
    # 5. Find usable rank column
    # -------------------------------------------------------------------------
    rank_candidates = [
        "rank_order",
        "rank",
    ...
    for c in df.columns:
        if "rank" in c and c not in rank_candidates:
    ...
    for c in rank_candidates:
        ...
        parsed = df[c].apply(parse_rank)
        valid_count = int(parsed.notna().sum())

        if valid_count > best_valid_count:
            best_valid_count = valid_count
            best_rank_col = c
            best_rank_values = parsed

    if best_rank_col is None or best_valid_count <= 0:
        return _placeholder_fig(
            "No usable rank column found. This chart requires subject rank data.",
            title=placeholder_title,
        )

    df["_rank"] = best_rank_values
    df = df.dropna(subset=["_rank"])

    if df.empty:
        return _placeholder_fig(
            "Subject ranks could not be parsed.",
            title=placeholder_title,
        )

    # -------------------------------------------------------------------------
    # 6. Match institutions appearing in both Engineering and Arts & Humanities
    # -------------------------------------------------------------------------
    df["_institution_key"] = df["name"].apply(normalize_institution_name)
    df = df[df["_institution_key"] != ""].copy()

    pivot = (
        df.pivot_table(
            index=["_institution_key", "country", "domain_group"],
            columns="domain",
            values="_rank",
            aggfunc="median",
        )
    ...
    if "Engineering" not in pivot.columns or "Arts & Humanities" not in pivot.columns:
        return _placeholder_fig(
            "Not enough universities appear in both Engineering and Arts & Humanities rankings.",
            title=placeholder_title,
        )

    pivot = pivot.dropna(subset=["Engineering", "Arts & Humanities"])

    if pivot.empty:
        return _placeholder_fig(
            "No institutions had both Engineering and Arts & Humanities ranks.",
            title=placeholder_title,
        )
    ...
    # -------------------------------------------------------------------------
    # 7. Summarise by chart-specific group
    # -------------------------------------------------------------------------
    summary = (
        pivot.groupby("domain_group")
        .agg(
            median_advantage=("engineering_advantage", "median"),
            n=("engineering_advantage", "count"),
        )
        .reindex(DOMAIN_GROUP_ORDER)
        .dropna(subset=["median_advantage"])
        .reset_index()
    )

    if summary.empty:
        return _placeholder_fig(
            "No Engineering advantage summary could be computed.",
            title=placeholder_title,
        )

    summary["n"] = summary["n"].astype(int)

    # Sort by value so negative groups appear on the left and positive groups on the right.
    summary = summary.sort_values("median_advantage", ascending=True)

    # Add sample size to y-axis labels
    summary["group_label"] = summary.apply(
        lambda r: f"{r['domain_group']} (n={int(r['n'])})",
        axis=1,
    )

    # Make zero values explicit so they do not look like missing bars
    summary["bar_label"] = summary["median_advantage"].apply(
        lambda v: "Balanced" if abs(v) < 0.5 else f"{v:+.0f}"
    )

    # Debug output
    print("\n[Domain Strategy] Using split Anglosphere diverging bar chart")
    print(f"[Domain Strategy] Rank column used: {best_rank_col}")

    if latest_year_text:
        print(f"[Domain Strategy] {latest_year_text}")

    print("[Domain Strategy] Median Engineering advantage by group:")
    print(
        summary[
            ["domain_group", "median_advantage", "n"]
    ...
    # -------------------------------------------------------------------------
    positive_groups = set(
        summary.loc[summary["median_advantage"] > 0, "domain_group"].tolist()
    )

    asia_row = summary[summary["domain_group"] == "Asia"]

    if positive_groups == {"Asia"}:
        chart_title = "Asia is the only group with a positive median Engineering advantage"
    elif (
        not asia_row.empty
        and asia_row["median_advantage"].iloc[0] == summary["median_advantage"].max()
        and asia_row["median_advantage"].iloc[0] > 0
    ):
        chart_title = "Asia tilts Engineering; Europe and Rest of World tilt Arts & Humanities"
    else:
        chart_title = "Median Engineering advantage by policy group"

    # -------------------------------------------------------------------------
    # 9. Build diverging horizontal bar chart
    # -------------------------------------------------------------------------
    max_abs = float(np.nanmax(np.abs(summary["median_advantage"])))
    axis_limit = max(10.0, max_abs * 1.35)

    fig = go.Figure()

    fig.add_trace(
        go.Bar(
            x=summary["median_advantage"],
            y=summary["group_label"],
            orientation="h",
            marker=dict(
                color=[
                    DOMAIN_GROUP_COLORS.get(group, "#94a3b8")
                    for group in summary["domain_group"]
                ],
                line=dict(color="white", width=1),
            ),
            text=summary["bar_label"],
            textposition="outside",
            cliponaxis=False,
            customdata=summary[["domain_group", "n"]].to_numpy(),
            hovertemplate=(
                "<b>%{customdata[0]}</b><br>"
                "Median Engineering advantage: %{x:+.1f} rank places<br>"
                "Universities compared: %{customdata[1]:.0f}<br><br>"
                "Positive = Engineering ranks better<br>"
                "Negative = Arts & Humanities ranks better"
                "<extra></extra>"
            ),
    ...
    # Add marker for balanced groups so zero does not look like missing data
    balanced_rows = summary[summary["median_advantage"].abs() < 0.5]

    if not balanced_rows.empty:
        fig.add_trace(
            go.Scatter(
                x=balanced_rows["median_advantage"],
                y=balanced_rows["group_label"],
                mode="markers",
                marker=dict(
                    symbol="diamond",
                    size=13,
                ...
                customdata=balanced_rows[["domain_group", "n"]].to_numpy(),
                hovertemplate=(
                    "<b>%{customdata[0]}</b><br>"
                    "Balanced median: %{x:+.1f}<br>"
                    "Universities compared: %{customdata[1]:.0f}"
                    "<extra></extra>"
                ),
                showlegend=False,
            )
    ...
    fig.add_annotation(
        text=(
            "Calculation: Engineering advantage = Arts & Humanities rank − Engineering rank. "
        ),
        x=0,
        y=-0.25,
        xref="paper",
    ...
    fig.update_layout(
        **_chart_layout(
            chart_title,
            height=600,
            margin=dict(l=220, r=120, t=100, b=150),
        ),
        xaxis=dict(
            title="Median Engineering advantage, rank places",
        ...
        yaxis=dict(
            title="",
            categoryorder="array",
            categoryarray=summary["group_label"].tolist(),
        ),
        bargap=0.35,
        showlegend=False,
    )
...
def build_html(
    ...
    sections += _section(
        ...
        heading="Asia shows the strongest Engineering advantage when the US and UK are separated",
        subtitle=(
                "This chart separates the United States and United Kingdom instead of aggregating them as Anglosphere. "
                "Engineering advantage is calculated as Arts & Humanities rank minus Engineering rank. "
                "Positive values mean Engineering ranks better."
            ),
        insights=[
            (
                "Asia is the clearest STEM specialist",
                "Asia is the only group with a substantively large Engineering advantage.",
            ),
            (
        ...
        story_title="Policy insight",
        story_body=(
            "Subject strategy is ranking strategy. Asia shows the clearest Engineering-oriented profile "
            "(+156 median rank places). The United States (+2), United Kingdom (-7), and other Anglosphere "
            "systems (+13) are close to balanced, suggesting comprehensive university models. Europe (-162) "
            "and the Rest-of-World group (-350) show relatively stronger Arts & Humanities positions. "
            "For policymakers, this means STEM specialisation can be a powerful route to ranking visibility, "
            "but it is not the only institutional strategy."
        ),
  ...
  <main id="top" class="page">
    ...
    <section id="conclusion" class="section">
      <div class="callout">
        ...
        <p>
          Policymakers should not treat global rank as a single neutral truth. Global rankings reward
          different routes to visibility: historical prestige, research capacity, national investment,
          and subject specialisation. A responsible ranking strategy should triangulate multiple systems,
          choose which mission and ranking logic to optimise for, and avoid reducing university quality
          to a narrow STEM-only agenda.
        </p>
      </div>
    </section>
  </main>
