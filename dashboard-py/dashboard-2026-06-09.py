...
6. Domain Strategy   — Asia is more Engineering-oriented
...
------------------------
    the_rankings.csv
    qs_rankings.csv
    the_subjects_2026.csv  or  the_subjects.csv

GDP data
--------
    Automatically fetched from World Bank API.
    Local gdp_per_capita.csv is optional, not required.
"""
...
def read_csv_auto(path: str | Path) -> pd.DataFrame:
    ...
    Useful because QS/THE are comma-separated, while subject data may be semicolon-separated.
    """
    return pd.read_csv(
        path,
        sep=None,
...
def first_column_containing(df: pd.DataFrame, tokens: list[str]) -> Optional[str]:
    """Return first column containing all tokens."""
    for c in df.columns:
...
def load_qs(path: str = "qs_rankings.csv") -> pd.DataFrame:
    ...
    # General fallback for columns like 2025_rank, 2026_rank, etc.
    if rank_col is None:
        year_rank_cols = [
...
def load_gdp(path: str | None = "gdp_per_capita.csv") -> pd.DataFrame:
    ...
    Otherwise, automatically fetch from World Bank API.
    """
...
def build_merged(the_df: pd.DataFrame, qs_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Adds rank_diff = |THE rank - QS rank|.
    """
...
def fig_methodology_divergence(merged: pd.DataFrame) -> go.Figure:
    """
    Boxplot of absolute THE-QS rank difference by region.
    Shows which regions are more sensitive to ranking methodology.
    """
    if merged.empty or "rank_diff" not in merged.columns:
        return _placeholder_fig(
    ...
    means = (
        merged.groupby("region")["rank_diff"]
        .mean()
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate="Regional mean difference: %{y:.1f}<extra></extra>",
        )
    )
    ...
    fig.update_layout(
        **_chart_layout(
        ...
        yaxis=dict(title="Absolute rank difference between THE and QS"),
        legend=dict(
            orientation="h",
            yanchor="bottom",
            y=-0.22,
...
def fig_the_driver_bar(the_df: pd.DataFrame, year: Optional[int] = None) -> go.Figure:
    ...
    fig.update_layout(
        **_chart_layout(
            ...
            height=560,
            margin=dict(l=190, r=70, t=90, b=130),
        ),
        xaxis=dict(
        ...
        annotations=[
            dict(
                text=(
                    "Methodology note: this is not a causal model. "
                    "It audits what THE operationally rewards in its overall score."
                ),
                x=0,
                y=-0.22,
                xref="paper",
...
def fig_gdp_outlier_strategy(the_df: pd.DataFrame, gdp_df: pd.DataFrame) -> go.Figure:
    ...
    df["bubble_size"] = np.clip(
        8 + 3.5 * np.sqrt(df["ranked_universities"]),
        8,
        38,
    )
    ...
    top_outliers = set(df.nlargest(min(3, len(df)), "outperformance")["country"])
    low_outliers = set(df.nsmallest(min(2, len(df)), "outperformance")["country"])

    label_set = important | top_outliers | low_outliers
...
# =============================================================================
# Chart 6 — Domain Strategy
# =============================================================================

def fig_engineering_advantage_box(subj_df: Optional[pd.DataFrame]) -> go.Figure:
    """
    Region-level Engineering vs Arts & Humanities advantage.
    ...
    if subj_df is None or subj_df.empty:
        return _placeholder_fig(
            ...
            title="Asia is more Engineering-oriented",
        )
    ...
    if value_type == "rank":
        ...
        y_title = "Engineering advantage: Arts rank − Engineering rank"
        subtitle = "Positive values mean Engineering ranks better than Arts & Humanities."
    else:
        pivot["engineering_advantage"] = (
            pivot["Engineering"] - pivot["Arts & Humanities"]
        )
        y_title = "Engineering advantage: Engineering score − Arts score"
        subtitle = "Positive values mean Engineering scores higher than Arts & Humanities."

    fig = go.Figure()

    for region in REGION_ORDER:
        sub = pivot[pivot["region"] == region].copy()
        ...
        fig.add_trace(
            go.Box(
                ...
                line=dict(color=REGION_COLORS[region]),
                hovertext=sub["name"],
                customdata=sub[["country"]].to_numpy(),
                hovertemplate=(
                    "<b>%{hovertext}</b><br>"
                    "Country: %{customdata[0]}<br>"
                    "Engineering advantage: %{y:.1f}"
                    "<extra></extra>"
                ),
                showlegend=False,
            )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate="Regional mean advantage: %{y:.1f}<extra></extra>",
        )
    )
    ...
    fig.update_layout(
        **_chart_layout(
            "Asia is more Engineering-oriented",
            height=650,
            margin=dict(l=80, r=50, t=90, b=130),
        ),
        xaxis=dict(
            title="Region",
            categoryorder="array",
...
def build_html(
    ...
    sections += _section(
        ...
        subtitle=(
            "This chart does not ask whether THE or QS is correct. It asks where the two systems "
            "disagree most, revealing which institutions are more methodology-sensitive."
        ),
        insights=[
            (
            ...
            (
                ...
                "Some regions experience larger THE-QS rank differences than others.",
            ),
            (
        ...
        tags=["THE", "QS", "Methodology sensitivity"],
    )
    ...
    sections += _section(
        ...
        story_title="Policy insight",
        story_body=(
            "THE operationalises excellence mainly through research capacity. This does not mean "
            "teaching or industry engagement are unimportant, but it does mean that they are weaker "
            "levers for improving THE rank compared with research strength."
        ),
    ...
    sections += _section(
        ...
        heading="Asia is more Engineering-oriented",
        subtitle=(
            "This chart compares each university's relative strength in Engineering versus Arts & Humanities, "
            "then summarises the pattern by region."
        ),
        insights=[
            (
                "Engineering advantage is regional",
                "Asian institutions show a stronger tilt toward Engineering than other regions.",
            ),
            (
                "Generalists remain important",
                "Anglosphere and European systems are more likely to preserve balanced subject portfolios.",
            ),
            (
                "STEM is a ranking strategy",
                "Engineering specialisation can be a faster route into research-heavy ranking systems.",
            ),
        ],
        fig=fig_domain,
        story_title="Policy insight",
        story_body=(
            "Subject strategy is ranking strategy. Policymakers should not blindly copy the Western "
            "comprehensive university model. For newer systems, STEM concentration may improve global "
            "visibility faster, but it can also narrow the broader public mission of the university."
        ),
      ...
      <div class="panel panel-pad">
        <div class="claim-grid">
          ...
          <div class="claim">
            ...
            <p>Asia is more Engineering-oriented.</p>
          </div>
        </div>
      </div>
    </section>
...
def main() -> None:
    ...
    fig_domain = fig_engineering_advantage_box(subj_df)
